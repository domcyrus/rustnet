...
在 Linux 5.13+ 上，RustNet 使用 [Landlock](https://landlock.io/) 在初始化后限制自身能力。这样即使包解析存在漏洞被利用，也能限制损害范围。
...
| 能力 | 任意 | pcap socket 打开后丢弃 `CAP_NET_RAW` |
| 能力 | 任意 | eBPF 程序加载后丢弃 `CAP_BPF`、`CAP_PERFMON` |
| 特权 | 任意 | `PR_SET_NO_NEW_PRIVS` 防止通过 setuid 二进制文件提升特权 |

### 工作原理

1. **初始化阶段**：RustNet 加载 eBPF 程序、打开包捕获句柄、创建日志文件
2. **能力剥离**：移除 `CAP_NET_RAW`、`CAP_BPF` 和 `CAP_PERFMON`
...
- 无法创建新的 raw socket（能力已剥离）
- 无法通过 setuid 二进制文件提升特权（`PR_SET_NO_NEW_PRIVS`）

### CLI 选项

```
--no-sandbox        禁用 Landlock 沙箱和能力剥离
...
| 平台 | 需求 |
|------|------|
| Linux | `CAP_NET_RAW` capability 或 root |
...
### 推荐：基于 Capability 的执行（Linux）

与其以 root 运行，不如仅授予所需的能力：
...
因此，强烈建议使用细粒度能力（`setcap cap_net_raw=eip`）运行，而不是以 root 运行。
