...
> **文档版本：** `main` 分支上的本指南描述开发中的代码，可能包含尚未发布的变更。使用 v1.6.0 时，请参阅 [v1.6.0 安全指南](https://github.com/domcyrus/rustnet/blob/v1.6.0/SECURITY.zh-CN.md)。运行 `rustnet --version` 查看已安装的版本。

RustNet 处理不受信任的网络数据，因此纵深防御至关重要。本文档描述了已实现的安全措施。

> **尚未发布的加固：** 请求的 UID/GID 降权失败时中止启动、保留安全打开的输出文件描述符且不为 Seatbelt 添加输出路径例外，以及检测输出文件冲突，均为 v1.6.0 之后的变更。下文的 Linux 启动 socket 清单也尚未发布。
