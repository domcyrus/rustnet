...
# 无需 sudo 运行（post-install 脚本已设置能力）
rustnet
...
**注意：** .deb 包通过 post-install 脚本自动设置 Linux 能力，因此你可以无需 sudo 运行 RustNet。
...
**注意：** .rpm 包通过 post-install 脚本自动设置 Linux 能力，因此你可以无需 sudo 运行 RustNet。
...
# 为 Homebrew 安装的二进制文件授予能力（现代内核 5.8+）
...
# 使用 eBPF 支持所需的 capability 运行（latest）
...
**注意：** 容器需要 capability（`NET_RAW`、`BPF` 和 `PERFMON`）或 privileged 模式才能进行带 eBPF 支持的数据包捕获。推荐使用主机网络（`--net=host`）以监控所有网络接口。
...
>
> **Linux：** 仅需要 **`CAP_NET_RAW`** capability —— **不需要**完整的 root 或 `CAP_NET_ADMIN`
> **最小权限原则：** 数据包捕获所需的最小权限
...
在 Linux 上，数据包捕获仅需要 `CAP_NET_RAW` capability 用于只读、非混杂数据包捕获。对于 eBPF 增强型进程追踪，需要额外的能力（`CAP_BPF` 和 `CAP_PERFMON`），但**不需要 `CAP_NET_ADMIN`**。
...
为二进制文件授予特定的网络能力，而无需完整的 root 特权：
...
**回退行为**：如果 eBPF 无法加载（例如能力不足、内核不兼容），应用会自动使用仅 procfs 模式。TUI 统计面板显示当前使用的检测方法：
...
- 检查能力是否已设置：`getcap $(which rustnet)` 或 `getcap ~/.cargo/bin/rustnet`
...
#### 操作不被允许（已设置能力）<a id="operation-not-permitted-with-capabilities-set"></a>

- 能力可能已被系统更新移除
- 重新应用能力（现代）：`sudo setcap 'cap_net_raw,cap_bpf,cap_perfmon+eip' $(which rustnet)`
- 某些文件系统不支持扩展属性（能力）
- 尝试将二进制文件复制到不同的文件系统（例如从 NFS 复制到本地磁盘）

#### 已设置能力但 eBPF 不可用<a id="ebpf-unavailable-despite-capabilities-being-set"></a>
...
内核会静默忽略位于以 `nosuid` 选项挂载的文件系统上的二进制文件的能力。常见 culprit：`/home` 在加固发行版上、`/tmp`、可移动介质、容器内的某些 bind-mount。

```bash
# 查找持有该二进制文件的挂载点并检查其选项
findmnt -T $(realpath $(which rustnet)) -o TARGET,OPTIONS
# 如果 OPTIONS 列包含 "nosuid"，能力将不会生效。
...
能力已授予，但内核返回了 `EPERM` 或 `EACCES`。三层可能阻止它 —— 按此顺序检查：
...
# 如果已列出，要么禁用该配置文件，要么添加一条规则允许 capability bpf、
# capability perfmon，以及该二进制文件的 bpf() 系统调用。

# 2c. unprivileged_bpf_disabled（Debian 设置为 =2；文件能力应能绕过）。
...
即使有文件能力，容器内的*bounding*集合也必须包含 `CAP_BPF` 和 `CAP_PERFMON`，否则它们在 exec 时会被屏蔽：
