...
80×24 では Activity のプロセス表と TX/RX の概要を表示します。高さが小さい場合は概要とプロセス行を縮め、80×12 でも現在の速度、キャプチャのカバー率、プロセスへの帰属率を表示します。Overview の System サイドバーには 90 列以上が必要です。幅が足りない場合は `i` でスクロール可能なシステム情報を重ねて表示します。`j`/`k`、Page Up/Down、マウスホイールでスクロールし、`i` または `Esc` で閉じます。接続の選択とフィルターは保持されます。

Details は幅が 100 列未満、またはコンテンツ領域が 24 行未満の場合にページ表示を使います（フィルター入力行や展開されたエラー欄がなければ、ターミナルの高さ 27 行が境界です）。`v` で Connection、Network、Process、Application、Health、Traffic を切り替えます。接続情報は上部の 1 行にまとめ、`j`/`k` で接続を切り替えてもページは保持します。収まらない内容は Ctrl+D/U またはマウスホイールでスクロールできます。大きいターミナルでは完全なダッシュボードを表示します。

Graph は幅が 100 列未満、またはコンテンツ領域が 32 行未満の場合、セクションを一つずつ表示します（フィルター入力行や展開されたエラー欄がなければ、ターミナルの高さ 35 行が境界です）。`v` で Traffic、Health、Distribution を切り替えます。現在のセクションとキー操作はグラフの上部に表示されます。大きいターミナルでは全セクションを表示し、再び縮めると前に選択したセクションに戻ります。
