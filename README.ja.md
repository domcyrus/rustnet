...
Overview、Details、Graph、Host は共通のセクション選択行を表示します。`[` / `]` で前 / 次のセクションへ移動し、名前のクリックでも選択できます。Tab / Shift+Tab と 1-5 はメインタブを切り替えます。これらの操作は `v`、Overview の `i`、Host の `s` / `i` を置き換えます。角括弧ではメインタブを切り替えません。ヘルプは引き続きオーバーレイで、表示中はセクション切り替えを受け付けません。

Overview は 90 列未満では Connections または System をページ内に表示します。幅が広い場合は両方のパネルを表示し、選択中の見出しに下線を付けます。System は `j` / `k`、Page Up/Down、マウスホイールでスクロールします。Esc で Connections に戻り、接続の選択とフィルターは保持します。

Details は 100 列未満またはコンテンツ領域が 24 行未満の場合、一つのセクションを表示します（フィルター入力行や展開されたエラー欄がなければターミナルの高さ 28 行が境界）。Connection、Network、Process、Application、Health、Traffic を選択できます。`j` / `k` で接続を切り替え、Ctrl+D/U またはマウスホイールで選択中の情報をスクロールします。大きいターミナルではダッシュボードを表示し、選択中のカードの見出しに下線を付けます。

Graph は 100 列未満またはコンテンツ領域が 32 行未満の場合、Traffic、Health、Distribution の一つを表示します（フィルター入力行や展開されたエラー欄がなければ高さ 36 行が境界）。大きいターミナルでは全セクションを表示し、選択中のセクションに下線を付けます。Host はすべてのサイズで Sockets と Interfaces を提供します。サイズ変更やメインタブの切り替えでも選択を保持します。
