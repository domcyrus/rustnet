...
Activity は 120 列未満またはコンテンツ領域が 25 行未満の場合（通常はターミナルの高さ 28 行が境界）、Processes と Capture を `v` / Shift+`v` で切り替えます。幅と高さに余裕があれば Capture を表の右側に表示します。Capture はスクロールでき、直近 60 秒の捕獲率と、保持中の通信量に対するプロセス帰属率を区別して表示します。インターフェースの一覧と速度表は Host にまとめ、Activity には捕獲率の計算に使うインターフェース名と元のカウンターを残します。

プロセス表は 1 行に 1 プロセスを表示し、10 件の表示制限はありません。矢印または `j` / `k`、Page Up/Down（Ctrl+B/F）、Home/End（`g` / `G`）、マウスホイールで全行を移動できます。スクロールバーと表示範囲が位置を示し、並べ替え、更新、サイズ変更でも選択したプロセスを維持します。Enter またはダブルクリックでページ内の詳細を開き、狭い表で省略された項目も確認できます。詳細はスクロール可能で、Esc で一覧に戻ります。対象が保持されなくなった場合は明示し、別のプロセスには切り替えません。50×12 でも一覧と Capture の全データにアクセスできます。
