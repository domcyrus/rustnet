...
impl MacOSProcessLookup {
    ...
    fn parse_lsof() -> Result<HashMap<ConnectionKey, (u32, String)>> {
        ...
        debug!("lsof header: {}", lines.get(0).unwrap_or(&""));
        ...
        for line in stdout.lines().skip(1) {
                ...
                match parts[4] {
                    "IPv4" | "IPv6" => {
                        ...
                        if parts.len() > 7 && (parts[7] == "TCP" || parts[7].contains("TCP")) {
                            ...
                            debug!("  No protocol detected from NODE field: {}", parts.get(7).unwrap_or(&""));
                            None
                        }
                    }
                    _ => {
            ...
            if let Some((protocol, local, remote)) = parse_lsof_connection_with_hint(connection_field, protocol_hint) {
                ...
                debug!("  Successfully parsed connection: {:?} -> {} ({})", key, process_name, pid);
        ...
        info!("Processed {} lines, successfully parsed {} connections", processed_lines, successful_parsers);
...
fn parse_lsof_connection_with_hint(name: &str, protocol_hint: Option<Protocol>) -> Option<(Protocol, SocketAddr, SocketAddr)> {
    // Parse lsof NAME field format:
    // "192.168.1.1:443->10.0.0.1:12345" (TCP)
    // "192.168.1.1:53" (UDP)
    // "*:80" (listening)
    
    debug!("    Parsing NAME field: '{}' with hint: {:?}", name, protocol_hint);
    ...
    if name.contains("->") {
        ...
        debug!("    Parsing arrow connection: '{}' -> '{}'", parts[0], parts[1]);
        ...
        debug!("    Success: {:?} {}:{} -> {}:{}", protocol, local.ip(), local.port(), remote.ip(), remote.port());
        ...
        debug!("    Success: {:?} {}:{} (listening/UDP)", protocol, local.ip(), local.port());
        Some((protocol, local, remote))
    } else {
        ...
        if ch == '\\' && chars.peek() == Some(&'x') {
            ...
            if hex_digits.len() == 2 {
                if let Ok(byte_val) = u8::from_str_radix(&hex_digits, 16) {
                    if let Some(decoded_char) = std::char::from_u32(byte_val as u32) {
...
mod tests {
    ...
    fn test_decode_lsof_string() {
        ...
        assert_eq!(
            decode_lsof_string("Brave\\x20Browser"),
            "Brave Browser"
        );

        // Test process name without escaping
        assert_eq!(
            decode_lsof_string("firefox"),
            "firefox"
        );

        // Test process name with single escaped space
        assert_eq!(
            decode_lsof_string("App\\x20Name"),
            "App Name"
        );
