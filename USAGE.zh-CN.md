...
进程浏览器可在出站 (TX) 和入站 (RX) 之间切换。表格和页面内详情提供以下信息：
...
表格每个进程占一行，不再限制为前十个进程。使用方向键或 `j` / `k`、Page Up/Down（或 Ctrl+B/F）、Home/End（或 `g` / `G`）及鼠标滚轮浏览所有保留的进程。滚动条和行范围显示当前位置；排序、实时更新和调整窗口大小时保持所选进程。

按 Enter 或双击进程打开页面内详情，窄屏表格省略的字段仍可在此查看。使用相同按键或滚轮滚动，Esc 返回所选进程。若进程已不再保留，详情会明确提示，不会自动显示其他进程。表格列和详情字段的含义如下：
...
宽屏将 Capture 放在进程表右侧。空间不足时，使用 `v` / Shift+`v` 或点击 Processes / Capture 切换区块。Capture 可滚动，调整窗口大小后保留位置，并明确区分最近 60 秒的捕获覆盖率与保留流量的进程归属比例，展示已归属、未知和总字节数。接口清单和速率表统一放在 Host，Activity 仅保留解释覆盖率所需的接口依据。流量占比条在空间允许时显示在同一行。
...
Activity 在不足 120 列或内容区域不足 25 行时显示 Processes 和 Capture 区块（无额外提示栏时为终端高度 28 行）。即使在 50×12 下也能滚动访问所有保留的进程。Capture 包含完整的覆盖率和归属信息，宽屏时显示在进程表旁边。
