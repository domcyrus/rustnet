...
- `Tab` 或 `]` —— 下一个标签页
- `Shift+Tab` 或 `[` —— 上一个标签页
- `1` / `2` / `3` / `4` / `5`：直接跳转到 概览 / 详情 / 活动 / 图表 / 帮助
- `Enter` —— 查看所选连接的详细信息
- `Esc` —— 返回上一个视图或清除活动过滤器
- `h` —— 切换帮助屏幕
...
- `x` —— 清除所有连接并重置统计（按两次确认）
- `t` —— 切换历史（已关闭）连接的显示
- `i`：在概览中切换 System 信息侧边栏，或在活动标签页切换接口详情
...
按 `d` 在出站 (TX，蓝色) 和入站 (RX，绿色) 之间切换，按 `s` 轮换活动指标的排序方式，按 `S` 反转排序顺序，按 `i` 切换详细接口表格。
...
前缀 `~` 表示该主机名是从 DNS 响应*推断*出来的，而非从连接本身提取。这对握手后的 QUIC 会话（SNI 已加密）以及不携带主机名载荷的纯 TCP/UDP 连接最有用。归因不需要主动查询，因此即使使用 `--no-resolve-dns` 也能工作。详情标签页会单独显示一行 **Attributed Name**，包含来源和观测时间（`~name (Captured DNS, 5s ago)`），使来源一目了然。归因主机名可以像其他主机名一样搜索：`sni:` / `host:` / `hostname:` 关键字过滤器和自由文本搜索都能匹配它们。
