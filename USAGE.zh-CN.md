...
Overview、Details、Activity 和 Graph 仅在完整布局放不下时显示共用的区块选择栏。按 `v` 选择下一个区块，Shift+`v` 选择上一个，也可点击名称。Host 在所有尺寸下使用 `v` / Shift+`v` 切换 Sockets 和 Interfaces。Tab / Shift+Tab、`[` / `]` 和 1-5 在所有页面切换主标签页。区块选择栏与主标签页共用标题和下划线样式，快捷键显示在底部。
