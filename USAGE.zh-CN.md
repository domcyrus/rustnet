...
在 80×24 终端中，Activity 保留进程表和 TX/RX 摘要。高度更小时，摘要和进程行会缩减，在 80×12 下仍显示当前速率、捕获覆盖率和进程归属比例。Overview 的 System 侧栏至少需要 90 列。宽度不足时，按 `i` 打开可滚动的系统信息浮层；使用 `j`/`k`、Page Up/Down 或鼠标滚轮滚动，按 `i` 或 `Esc` 关闭，保留当前连接选择和筛选条件。

Details 在宽度不足 100 列或内容区域不足 24 行时使用分页（没有筛选输入行或展开的错误栏时，对应终端高度 27 行）。按 `v` 在 Connection、Network、Process、Application、Health 和 Traffic 之间切换。顶部保留一行连接信息；`j`/`k` 切换连接时保留当前页面，Ctrl+D/U 或鼠标滚轮可滚动放不下的内容。更大的终端保留完整仪表板。

Graph 在宽度不足 100 列或内容区域不足 32 行时，每次显示一个区块（没有筛选输入行或展开的错误栏时，对应终端高度 35 行）。按 `v` 在 Traffic、Health 和 Distribution 之间循环切换。图表上方会显示当前区块和快捷键。更大的终端会同时显示所有区块；缩小后会恢复上次选择的紧凑区块。
