...
ドロップ数がゼロでスナップショットの世代番号が連続していても、短時間の接続や
エンドポイントを再利用する接続が一度も出力されない場合があります。全体の
パケット数は接続ごとの完全な履歴を保証しません。パケット単位の記録が必要なら
`ExecStart` に `--pcap-export /var/lib/rustnet/capture.pcap` を追加し、stdout と
自動生成される PCAP サイドカーを含む他の出力とは別のファイルにしてください。
キャプチャファイルは起動ごとに切り詰められるため、再起動前にサービスを停止した
状態で保存してください。PCAP もキャプチャフィルター、キャプチャ長、ドロップの
影響を受けます。網羅性を検証する際はドロップ数と独立した参照キャプチャを確認します。
`stats.pcap_export_errors` がゼロで終了処理が成功していることも確認し、
`stats.pcap_records_written` と保存ファイルを照合してください。ドロップ数が
ゼロでも、書き込みやフラッシュの失敗でファイルが不完全になる場合があります。

5 秒間隔は設定例であり、処理能力の保証ではありません。接続数が多いと完全な
スナップショットは CPU と保存容量を大きく消費します。無人運用を有効にする前に
実際の通信と出力の増加量を測定してください。出力頻度を下げてもバックグラウンド
収集の負荷はなくならず、スナップショットが完全な履歴になるわけではありません。
